<script>
    ...
    function waveformAction(node) {
        ...
        return {
            destroy() {
                // Ensure we don't nullify if a newer instance already claimed the container
                setTimeout(() => {
...
<div class="flex min-h-screen flex-col bg-[#050505] text-gray-100 selection:bg-purple-500/30 font-instrument-sans pt-[max(env(safe-area-inset-top),1rem)]">
    ...
    <div 
        ...
        class:translate-y-[120%]={!player.currentTrack}
        class:translate-y-0={!!player.currentTrack}
        class:opacity-0={!player.currentTrack}
        class:opacity-100={!!player.currentTrack}
        aria-hidden={!player.currentTrack}
    >
        <div class="mx-auto max-w-md overflow-hidden rounded-2xl border border-white/5 bg-black/60 shadow-2xl backdrop-blur-xl supports-[backdrop-filter]:bg-black/40">
            <!-- Waveform / Progress -->
            <div class="relative px-3 pt-3">
                ...
                {:else}
                    <div
                        use:waveformAction
                        class="w-full"
                        class:opacity-0={!player.hasWaveform}
                        class:opacity-100={player.hasWaveform}
                    ></div>
                {/if}
            </div>
            ...
            <div class="flex items-center gap-4 px-3 pb-3 pt-1">
                <!-- Artwork -->
                <div class="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl bg-gradient-to-br from-gray-800 to-gray-900 shadow-inner">
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-semibold tracking-tight text-white/90">
                ...
                <!-- Time -->
                <span class="text-[10px] font-medium tabular-nums text-white/30 shrink-0">
                ...
                >
                    ...
                    {:else}
                        <svg xmlns="http://www.w3.org/2000/svg" class="ml-0.5 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
